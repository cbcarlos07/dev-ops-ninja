...
```sh
$ cd redis
$ docker build -t cbcarlos07/redis:devops .
$ docker run -d --name redis -p 6379:6379 cbcarlos07/redis:devops
$ docker ps
$ docker logs redis
```
...
```sh
$ cd ../node
$ docker build -t cbcarlos07/node:devops .
```
Agora iremos rodar a imagem do node, fazendo a ligação dela com o container do Redis.
```sh
$ docker run -d --name node -p 8080:8080 --link redis cbcarlos07/node:devops
$ docker ps 
$ docker logs node
```
...
```sh
$ cd ../nginx
$ docker build -t cbcarlos07/nginx:devops .
```
Criando o container do nginx a partir da imagem e fazendo a ligação com o container do Node
```sh
$ docker run -d --name nginx -p 80:80 --link node cbcarlos07/nginx:devops
$ docker ps
```
...
- Linha 8 = cbcarlos07/nginx:devops
- Linha 18 = image: cbcarlos07/redis:devops
- Linha 37 = image: cbcarlos07/node:devops
